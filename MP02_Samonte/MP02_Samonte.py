...
def extract_input():
    ...
    with open("MP02 Checker.txt", "r", encoding="utf-8") as f:
...
def get_process(processes, arrival_ctr):
    ...
    return sorted(eligible, key=operator.itemgetter(3))[0] if eligible else None
...
def append_to_file():
    ...
    try:
        with open("MP02 Checker.txt", "a", encoding="utf-8") as f:
            f.write('\n')
            f.write(input_str)
            f.write(gantt_str)
            f.write(table_str)
...
def main():
    while True:
        ...
        output = priority_preemptive(processes)
